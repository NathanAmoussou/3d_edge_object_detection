...
\begin{titlepage}
    ...
    {\scshape\LARGE Université Côte d'Azur \par}
    \vspace{1.5cm}
    ...
    % --- Modification : Plus d'italique (\itshape retiré) ---
    {\Large
    \textbf{Auteurs :} \\ 
    Noé Florence, Nathan Amoussou, Louis Malmassary \par
    }
    
    \vspace{1cm}

    {\large \textbf{Professeur :} \\  Gerald Rocher\par}
    
    \vspace{1.5cm}

    % --- Ajout : Section Lien GitHub ---
    % Nécessite \usepackage{hyperref} ou \usepackage{url} dans le préambule
    {\large \textbf{Dépôt GitHub :} \\ 
    \url{https://github.com/NathanAmoussou/3d_edge_object_detection} \par}
    
    \vfill

    % Ajout de la date (facultatif, sinon enlever la ligne suivante)
    {\large \today \par} 
...
\begin{itemize}
    ...
    Pour les tests avec la oak-d pro qui vont suivre, la caméra sera pilotée par notre ordinateur, via USB.
    % TODO : Vérifier cette dernière phrase.
\end{itemize}
...
\subsection{Optimisations au \textit{runtime} }
...
\begin{itemize}
  \item \textbf{Pi, i9, 4070 --- Fusion de graphe} : \{\texttt{DISABLE\_ALL}, \texttt{ENABLE\_BASIC}, \texttt{ENABLE\_ALL}\}.
  ...
  Le gain est en général plus marqué sur CPU, où réduire les allocations, copies et surcoûts d’exécution améliore directement les performances et la pression mémoire. Sur GPU via ORT, les fusions peuvent aussi aider en réduisant des lancements de kernels et des passages mémoire, mais l’effet peut être moins net si le goulot principal est ailleurs (pré/post-traitement CPU, transferts). 
\end{itemize}
...
\section{Résultats et analyse} \label{sec:resultats}

Cette section analyse les résultats obtenus avec le protocole de \cref{subsec:protocole_de_benchmarking}, en lien direct avec les leviers d’optimisation introduits en \cref{sec:benchmarking} et les contraintes matérielles décrites en \cref{sec:contexte}. 

La \cref{fig:lat_reso_barchart} est un histogramme comparant la latence E2E p95 (ms) en fonction de la résolution d’entrée (px), pour trois plateformes \textit{edge} (Orin, Oak, Pi). L’axe des abscisses correspond aux résolutions testées (256, 320, 416, 512, 640) et l’axe des ordonnées à la latence p95.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{fig1_resolution_e2e.png}
    \caption{Latence en fonction de la résolution d'entrée, par plateformes \textit{edges}}
    \label{fig:lat_reso_barchart}
\end{figure}

La \cref{fig:lat_quantif_barchart} regroupe deux histogrammes : (i) plateformes \textit{edge} et (ii) plateformes 4070 et i9. L’axe des abscisses correspond à la plateforme, l’axe des ordonnées à la puissance moyenne (W), et chaque paire de barres compare FP16 à FP32.
...
\bottomrule
\end{tabular}
\caption{Effet de FP16 sur la puissance moyenne (valeurs indiquées sur \cref{fig:lat_quantif_barchart}).}
\label{tab:fp16_power_values}
\end{table}

\begin{figure}[H]
    \centering
    % --- Première sous-figure (Gauche) ---
    \begin{subfigure}{0.4\textwidth}
        \centering
        % On met width=\linewidth pour que l'image prenne toute la largeur de la sous-figure
        \includegraphics[width=\linewidth]{fig3a_quant_power_edge.png}
        \caption{Plateformes \textit{edge}}
        \label{fig:edge}
    \end{subfigure}
    \hfill % Ajoute un espace flexible entre les deux images
    % --- Deuxième sous-figure (Droite) ---
    \begin{subfigure}{0.4\textwidth}
        \centering
        \includegraphics[width=\linewidth]{fig3b_quant_power_desktop.png}
        \caption{Plateformes 4070 et i9}
        \label{fig:desktop}
    \end{subfigure}
    
    % --- Légende globale et Label ---
    \caption{Puissance électrique en fonction du niveau de quantification, par plateformes}
    \label{fig:lat_quantif_barchart}
\end{figure}

La \cref{fig:lat_fus_barchart} est un histogramme comparant la latence E2E p95 (ms) en fonction du niveau de fusion ORT (\texttt{DISABLE\_ALL}, \texttt{ENABLE\_BASIC}, \texttt{ENABLE\_ALL}) sur 4070, Pi et i9. L’axe des abscisses est la plateforme, l’axe des ordonnées la latence, et les couleurs correspondent aux niveaux de fusion.

On observe que \texttt{ENABLE\_ALL} a un impact très fort sur i9, un impact faible sur Pi, et un impact négligeable sur 4070. Cela confirme l’hypothèse que les fusions ORT sont surtout pertinentes quand l’exécution est dominée par le CPU, alors qu’en régime accéléré (GPU) la latence est davantage contrainte par d’autres éléments (orchestration, transferts, kernels).
...
\begin{tabular}{lcc}
\toprule
Plateforme & Gain relatif avec \texttt{ENABLE\_ALL} & Lecture \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Impact du niveau de fusion ORT sur la latence (valeurs indiquées sur \cref{fig:lat_fus_barchart}).}
\label{tab:fusion_ort_values}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{fig4_fusion_ort.png}
    \caption{Latence en fonction du niveau de fusion, par plateformes}
    \label{fig:lat_fus_barchart}
\end{figure}

La \cref{fig:lat_puiss_elag_reso_shaves} est un nuage de points (échelles log) reliant puissance (W, axe $x$) et latence E2E p95 (ms, axe $y$) sur Oak. La figure encode la résolution (forme), la taille du modèle (taille du marqueur) et le nombre de SHAVEs (couleur). La tendance principale est structurée par la résolution et la taille du modèle : à résolution plus élevée et modèle plus grand, la latence augmente fortement. Le nombre de SHAVEs agit comme réglage plus fin : l’effet n’est pas strictement monotone, et reste secondaire face à la résolution et au niveau d’élagage.
...
% \toprule
% Option Orin & Effet attendu & Observation sur nos mesures \textit{runtime} \\
% \midrule
% \textit{Heuristic} (H0/H1) & accélérer la phase de build & pas de gain systématique sur E2E/power \\
% \textit{Sparsity} (SP0/SP1) & gain si sparsité structurée compatible & pas de gain net observé sur E2E/power \\
% \bottomrule
% \end{tabular}
% \caption{Tendances négatives sur Orin : options TensorRT sans effet \textit{runtime} robuste dans nos tests.}
% \label{tab:orin_negative_specific}
% \end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{fig_oak_shaves.png}
    \caption{Compromis latence/puissance électrique en fonction de du niveau d'élagage et de résolution, et du nombre de SHAVEs, sur Oak}
    \label{fig:lat_puiss_elag_reso_shaves}
\end{figure}

Pour Orin, la \cref{fig:lat_puisse_lag_reso_quantif} confirme que les points se structurent d’abord par résolution et taille de modèle, et que FP16 tend à améliorer le compromis énergétique. En revanche, nos options spécifiques TensorRT (mode \textit{heuristic} et \textit{sparsity}, \cref{tab:merged_metrics}) ne montrent pas de tendance robuste sur les métriques \textit{runtime} (latence E2E p95 et puissance moyenne) au-delà de la variabilité : c’est une tendance négative importante à rapporter, cohérente avec le fait que (i) \textit{heuristic} vise principalement le temps de construction de l’engine et (ii) \textit{sparsity} requiert une sparsité structurée absente de nos variantes.
...
\begin{tabular}{lcccc}
\toprule
& \multicolumn{2}{c}{\textit{Heuristic}} & \multicolumn{2}{c}{\textit{Sparsity}} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
Métrique & H0 & H1 & SP0 & SP1 \\
\midrule
E2E p95 (ms)   & \textbf{47.08} & 47.42          & 47.31          & \textbf{47.19} \\
mAP@50 (\%)    & \textbf{66.32} & 66.3           & \textbf{66.31} & 66.3           \\
GPU util (\%)  & \textbf{45.88} & 46.23          & 46.67          & \textbf{45.44} \\
Puissance moyenne (W) & 1.87           & \textbf{1.84}  & 1.87           & \textbf{1.84}  \\
\bottomrule
\end{tabular}
\caption{Comparaison des mesures en fonction des niveaux d'\textit{heuristic} et de \textit{sparsity}}
\label{tab:merged_metrics}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{fig_orin_detailed.png}
    \caption{Compromis latence/puissance en fonction du niveau d'élagage, de résolution et de quantification, sur Orin}
    \label{fig:lat_puisse_lag_reso_quantif}
\end{figure}


La \cref{fig:prec_lat_plat_elag_reso} représente la précision mAP@50 (axe $y$) en fonction de la latence E2E p95 (ms, axe $x$ en log), en distinguant plateformes, tailles de modèles et résolutions. Cette figure synthétise le compromis global : la précision augmente globalement avec la résolution et la taille du modèle, au prix d’une latence plus élevée.

On observe une séparation claire des régimes : 4070 se situe dans la zone faible latence avec précision élevée, Orin et i9 forment un compromis intermédiaire, tandis que Pi et Oak sont décalés vers des latences nettement plus élevées pour des niveaux de précision comparables. Ce résultat met en évidence que, sur les cibles \textit{edge}, la contrainte de latence impose plus souvent de réduire résolution et taille du modèle, ce qui limite la précision atteignable à latence acceptable.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{fig_global_latency_precision.png}
    \caption{Compromis précision/latence en fonction de la plateforme, du niveau d'élagage et de résolution}
    \label{fig:prec_lat_plat_elag_reso}
\end{figure}

Les \cref{fig:puiss_lat_pi,fig:puiss_lat_orin,fig:puiss_lat_oak} montrent, pour Pi, Orin et Oak, un nuage de points puissance moyenne (W, axe $y$) vs latence E2E p95 (ms, axe $x$), et mettent en évidence les variantes non dominées (front de Pareto). Les points annotés correspondent aux configurations candidates selon l’objectif (minimiser la latence à puissance donnée, ou minimiser la puissance à latence donnée).

Sur Pi, le front met en avant des variantes \texttt{n/256} comme meilleures en latence, avec quelques compromis à plus haute résolution. Sur Orin, le front est réduit et souligne surtout l’intérêt de FP16 à latence comparable. Sur Oak, un candidat \texttt{n/256/FP16} avec un nombre intermédiaire de SHAVEs ressort comme compromis favorable, ce qui est cohérent avec l’analyse précédente (SHAVEs agit surtout en réglage fin).

\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{fig_pareto_pi.png}
    \caption{Meilleures variantes évaluées sur Pi par rapport au compromis puissance électrique/latence}
    \label{fig:puiss_lat_pi}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{fig_pareto_orin.png}
    \caption{Meilleures variantes évaluées sur Orin par rapport au compromis puissance électrique/latence}
    \label{fig:puiss_lat_orin}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{fig_pareto_oak.png}
    \caption{Meilleures variantes évaluées sur Oak par rapport au compromis puissance électrique/latence}
    \label{fig:puiss_lat_oak}
\end{figure}

\section{Conclusion} \label{sec:analyse_et_conclusion}

Ce projet avait pour objectif d’étudier, dans un cadre reproductible, comment différentes couches d’optimisation d’un détecteur d’objets (YOLOv11) interagissent avec des plateformes hétérogènes, en particulier \textit{edge}. Pour cela, nous avons construit un \textit{workflow} unifié (export, compilation, exécution) et un protocole de \textit{benchmark} commun, mesurant principalement la latence \textit{end-to-end} (p95), la puissance moyenne au niveau système, et la précision (mAP@50), tout en instrumentant l’exécution pour interpréter les résultats.

Les résultats montrent que les leviers les plus structurants sont ceux qui modifient directement la complexité du problème : la résolution d’entrée et la taille du modèle. Sur les cibles contraintes (Pi et Oak), la résolution domine largement la latence, avec une dégradation forte lorsque l’on monte en 512--640 px, tandis que l’Orin maintient un régime nettement plus rapide grâce à l’accélération GPU. La quantification FP16 améliore nettement le compromis énergétique sur les plateformes accélérées (Orin, 4070), alors que l’effet est faible ou négligeable sur CPU (i9 et, dans une moindre mesure, Pi). Côté \textit{runtime}, les optimisations de fusion ORT apportent un gain important sur i9, mais restent limitées sur Pi et quasi nulles sur 4070, ce qui confirme que leur intérêt dépend fortement du goulot (overhead d’exécution vs calcul accéléré). Enfin, les optimisations spécifiques plateforme se comportent comme attendu : sur Oak, le nombre de SHAVEs agit surtout comme réglage fin derrière résolution/élagage, et sur Orin les options \textit{heuristic} et \textit{sparsity} n’ont pas montré d’impact \textit{runtime} robuste dans nos conditions, ce qui suggère que leur effet principal est ailleurs (notamment sur le \textit{build time}) ou conditionné à des prérequis (sparsité structurée).

Ces observations permettent de proposer, par plateforme, des configurations candidates via une lecture multi-objectif (fronts de Pareto puissance/latence) : (i) sur Pi et Oak, des variantes petites et basse résolution constituent la base la plus réaliste pour respecter des contraintes temps réel, (ii) sur Orin, FP16 domine généralement FP32 à latence comparable, et (iii) sur i9/4070, les choix de \textit{runtime} (fusions) et de précision peuvent surtout être guidés par des objectifs énergétiques plutôt que par la latence brute.

Plusieurs limites doivent toutefois être soulignées : la comparabilité des métriques d’occupation accélérateur n’est pas parfaite (en particulier sur Oak), et le jeu de données coco128 réduit le temps d’évaluation mais peut limiter la stabilité statistique de certaines métriques. En ouverture, un prolongement naturel consisterait à (i) ajouter explicitement les métriques manquantes liées au déploiement (énergie par image, débit, mémoire, \textit{time-to-first-run} et temps de compilation des moteurs), (ii) étendre l’étude à INT8 (calibration) et à des optimisations structurelles (pruning/sparsité structurée) pour rendre effectives certaines options matérielles, et (iii) mieux caractériser les effets système (température, \textit{throttling}, multi-flux, contention CPU pré/post-traitement, transfert USB sur Oak) afin de rapprocher encore les mesures des contraintes d’une intégration \textit{IoT-CPS} réelle.
